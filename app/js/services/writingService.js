      ...
      function alphabetWrite (language, word, writingSystem) {
        ...
        for (let syllableIndex in word.syllables) {
          ...
          for (let phonemeIndex in syllable.phonemes) {
            ...
            writtenWord += writeSymbolOfPhoneme(language, writingSystem, word, phoneme, phonemeIndex, syllableIndex)
          }
        }
        return writtenWord
      }

      function writeSymbolOfPhoneme (language, writingSystem, word, phoneme, phonemeIndex, syllableIndex) {
        if (phoneme.length === 0) {
          return ''
        }
